...
def get_action_mask_from_state(state):
    ...
    for _col in range(col):
        _start_mask = num_act_charge_actions + (MAX_NUM_ATOMS * _col + _col) * len(BOND_NAMES)
        _end_mask = num_act_charge_actions + MAX_NUM_ATOMS * (_col + 1) * len(BOND_NAMES)
        action_vec_mask[_start_mask:_end_mask] = 1
    _start_mask = num_act_charge_actions + (MAX_NUM_ATOMS * col + col) * len(BOND_NAMES)
    action_vec_mask[_start_mask:] = 1
    ...
    for _col in range(col + 1):
        ...
        remaining_valence_col = state[_col, _col, -1]
        for row in range(_col):
            remaining_valence_row = state[row, row, -1]
            min_remaining_valence = np.minimum(remaining_valence_row,
                                               remaining_valence_col)
            cell_has_bond = state[row, _col, :-1].sum(-1) == 3
            cell_idx = row + _col * MAX_NUM_ATOMS
            # if cell has bond, do not allow bond creation
            if cell_has_bond:
                mask_start = start_idx + cell_idx * len(BOND_NAMES)
                mask_end = start_idx + (cell_idx + 1) * len(BOND_NAMES)
            ...
            if has_invalid_bond:
                mask_start = int(start_idx + cell_idx * len(BOND_NAMES) + min_remaining_valence + 1)
                mask_end = int(start_idx + (cell_idx + 1) * len(BOND_NAMES))
                action_vec_mask[mask_start:mask_end] = 1
    ...
    for jj in range(gragh_dim):
        ...
        for ii in range(jj):
            ...
            if sum(smi_graph[ii, jj, :]) == 3:
                ...
                loc_act_idx = jj * MAX_NUM_ATOMS + ii
