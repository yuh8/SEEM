...
from src.data_process_utils_unres import (get_action_mask_from_state,
...
def update_state_with_action(action_logits, state, num_atoms):
    ...
    if action_idx <= num_act_charge_actions:
        ...
        cell_idx = (action_idx - num_act_charge_actions) // len(BOND_NAMES)
        col_idx = cell_idx // MAX_NUM_ATOMS
...
if __name__ == "__main__":
    ...
    model = load_json_model("generator_model_random_mol_unres_2022-04-19/generator_model_random_mol_unres.json")
    model.compile(optimizer='adam',
                  loss=keras.losses.CategoricalCrossentropy(from_logits=True),
                  metrics=[keras.metrics.CategoricalAccuracy()])
    model.load_weights("./checkpoints/generator_2022-04-19/")
    gen_samples_df = []
    count = 0
